...
tensor<_Tp> tensor<_Tp>::matmul(const tensor& __other) const {
  ...
  const int __blocksPerGrid =
      (__ret_sh[0] * __ret_sh[1] + __threadsPerBlock - 1) / __threadsPerBlock;
  ...
  cudaMemcpy(__d_a, this->__data_.data(), this->__data_.size() * sizeof(value_type),
             cudaMemcpyHostToDevice);
  cudaMemcpy(__d_b, __other.__data_.data(), __other.__data_.size() * sizeof(value_type),
             cudaMemcpyHostToDevice);

  matmul_kernel<<<blocksPerGrid, threadsPerBlock>>>(__d_a, __d_b, __d_c, this->__shape_[0],
...
#if defined(__ARM_NEON)
  using neon_type =
      typename std::conditional<std::is_same<value_type, _f32>::value, neon_f32, void>::type;
...
inline const tensor<_Tp>& tensor<_Tp>::clipped_relu_(const value_type __clip_limit) const {
  return this->clipped_relu_(__clip_limit);
}
...
inline const tensor<_Tp>& tensor<_Tp>::clamp_(const_pointer __min_val,
  ...
  return this->clamp_(__min_val, __max_val);
}
