...
template<typename T>
struct narrow_neon_type_selector;

template<>
struct narrow_neon_type_selector<_s32>
{
    using type = int32x2_t;
};

template<>
struct narrow_neon_type_selector<_u32>
{
    using type = uint32x2_t;
};

template<>
struct narrow_neon_type_selector<_f32>
{
...
template<typename T>
neon_type<T> neon_vcgtq(neon_type<T>& a, neon_type<T>& b) {
    if constexpr (std::is_same_v<T, _f16>)
    {
