...
tensor<bool> tensor<_Tp>::neon_equal(const tensor& other) const {
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_f32 data_vec1  = vld1q_f32(reinterpret_cast<const float*>(&this->data_[i]));
    ...
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_u32 data_vec1  = vld1q_u32(reinterpret_cast<const uint32_t*>(&this->data_[i]));
...
tensor<bool> tensor<_Tp>::neon_equal(const value_type val) const {
  ...
  std::vector<bool> ret(this->data_.size());
  index_type        i        = 0;
  const index_type  simd_end = this->data_.size() - (this->data_.size() % 4);

  if constexpr (std::is_floating_point_v<value_type>) {
    float32x4_t val_vec = vdupq_n_f32(val);

    for (; i < simd_end; i += 4) {
      float32x4_t data_vec   = vld1q_f32(reinterpret_cast<const float*>(&this->data_[i]));
      uint32x4_t  cmp_result = vceqq_f32(data_vec, val_vec);

      uint32_t results[4];
      vst1q_u32(results, cmp_result);  // Store results into an array
      for (int j = 0; j < 4; ++j) ret[i + j] = results[j] != 0;
    }
  } else if constexpr (std::is_signed_v<value_type>) {
    int32x4_t val_vec = vdupq_n_s32(val);

    for (; i < simd_end; i += 4) {
      int32x4_t  data_vec   = vld1q_s32(reinterpret_cast<const int32_t*>(&this->data_[i]));
    ...
    for (; i < simd_end; i += 4) {
      uint32x4_t data_vec   = vld1q_u32(reinterpret_cast<const uint32_t*>(&this->data_[i]));
...
tensor<bool> tensor<_Tp>::neon_less_equal(const tensor& other) const {
  ...
  if constexpr (std::is_same_v<value_type, _f32>) {
    for (; i < vs; i += _ARM64_REG_WIDTH) {
      neon_f32 va       = vld1q_f32(this->data_.data() + i);
    ...
    for (; i < vs; i += _ARM64_REG_WIDTH) {
      neon_s32 va       = vld1q_s32(this->data_.data() + i);
    ...
    for (; i < vs; i += _ARM64_REG_WIDTH) {
      neon_u32 va       = vld1q_u32(this->data_.data() + i);
...
tensor<bool> tensor<_Tp>::neon_less_equal(const value_type val) const {
  ...
  size_t vector_size = this->data_.size() / _ARM64_REG_WIDTH * _ARM64_REG_WIDTH;

  if constexpr (std::is_same_v<value_type, _f32>) {
    for (; i < vector_size; i += _ARM64_REG_WIDTH) {
      neon_f32 vec_a    = vld1q_f32(this->data_.data() + i);
    ...
    for (; i < vector_size; i += _ARM64_REG_WIDTH) {
      neon_s32 vec_a    = vld1q_s32(this->data_.data() + i);
    ...
    for (; i < vector_size; i += _ARM64_REG_WIDTH) {
      neon_u32 vec_a    = vld1q_u32(this->data_.data() + i);
