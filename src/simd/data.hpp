...
tensor<_Tp>& tensor<_Tp>::neon_zeros_(shape_type sh) {
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    for (; i < simd_end; i += _ARM64_REG_WIDTH) vst1q_f32(&this->data_[i], zero_vec);
  } else if constexpr (std::is_signed_v<value_type>) {
    neon_s32 zero_vec = vdupq_n_s32(0);
    for (; i < simd_end; i += _ARM64_REG_WIDTH) vst1q_s32(&this->data_[i], zero_vec);
  } else if constexpr (std::is_unsigned_v<value_type>) {
    neon_u32 zero_vec = vdupq_n_u32(0);
    for (; i < simd_end; i += _ARM64_REG_WIDTH) vst1q_u32(&this->data_[i], zero_vec);
  }
#pragma omp parallel
  for (; i < s; ++i) this->data_[i] = value_type(0.0);
...
#pragma omp parallel
  for (; i < s; ++i) this->data_[i] = value_type(1.0);
...
tensor<_Tp>& tensor<_Tp>::neon_randomize_(const shape_type& sh, bool bounded) {
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    for (; i + _ARM64_REG_WIDTH <= static_cast<index_type>(s); i += _ARM64_REG_WIDTH) {
      ...
      if (!bounded) random_values = vmulq_f32(random_values, vrecpeq_f32(scale));
