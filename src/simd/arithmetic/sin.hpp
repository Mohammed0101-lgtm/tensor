...
tensor<_Tp>& tensor<_Tp>::neon_sin_() {
  ...
  if constexpr (std::is_same_v<value_type, _f32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      vst1q_f32(&this->data_[i], sin_vec);
    }
    ...
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      vst1q_s32(&this->data_[i], sin_vec);
    }
    ...
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      vst1q_u32(&this->data_[i], sin_vec);
    }
  }
#pragma omp parallel
...
tensor<_Tp>& tensor<_Tp>::neon_sinc_() {
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      neon_f32 sinc_v =
          vbslq_f32(vcgeq_f32(vabsq_f32(v), vdupq_n_f32(1e-6f)),  // Check |x| > epsilon
                    vdivq_f32(vsinq_f32(pi_v), pi_v),  // sinc(x) = sin(pi * x) / (pi * x)
                    vdupq_n_f32(1.0f));                // sinc(0) = 1

      vst1q_f32(reinterpret_cast<_f32*>(&this->data_[i]), sinc_v);
    }
  }
#pragma omp parallel
  for (; i < this->data_.size(); ++i) {
...
tensor<_Tp>& tensor<_Tp>::neon_sinh_() {
  ...
  if constexpr (std::is_same_v<value_type, _f32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      vst1q_f32(&this->data_[i], sinh_vec);
    }
  } else if constexpr (std::is_same_v<value_type, _s32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_s32 data_vec = vld1q_s32(reinterpret_cast<const _s32*>(&this->data_[i]));
      ...
      vst1q_s32(&this->data_[i], sinh_vec);
    }
  } else if constexpr (std::is_same_v<value_type, _u32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_u32 data_vec = vld1q_u32(reinterpret_cast<const _u32*>(&this->data_[i]));
      ...
      vst1q_u32(&this->data_[i], sinh_vec);
    }
  }
#pragma omp parallel
  for (; i < this->data_.size(); ++i) {
...
tensor<_Tp>& tensor<_Tp>::neon_asinh_() {
  ...
  if constexpr (std::is_same_v<value_type, _f32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      vst1q_f32(reinterpret_cast<_f32*>(&this->data_[i]), asinh_vec);
    }
  } else if constexpr (std::is_same_v<value_type, _s32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_s32 data_vec = vld1q_s32(reinterpret_cast<const _s32*>(&this->data_[i]));
      ...
      vst1q_s32(reinterpret_cast<_s32*>(&this->data_[i]), asinh_vec);
    }
  } else if constexpr (std::is_same_v<value_type, _u32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_u32 data_vec = vld1q_u32(reinterpret_cast<const _u32*>(&this->data_[i]));
      ...
      vst1q_u32(reinterpret_cast<_u32*>(&this->data_[i]), asinh_vec);
    }
  }
#pragma omp parallel
  for (; i < this->data_.size(); ++i) {
...
tensor<_Tp>& tensor<_Tp>::neon_asin_() {
  ...
  if constexpr (std::is_same_v<value_type, _f32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      ...
      vst1q_f32(&this->data_[i], asin_vec);
    }
  } else if constexpr (std::is_same_v<value_type, _s32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_s32 data_vec = vld1q_s32(reinterpret_cast<const _s32*>(&this->data_[i]));
      ...
      vst1q_s32(&this->data_[i], asin_vec);
    }
  } else if constexpr (std::is_same_v<value_type, _u32>) {
    for (; i < simd_end; i += _ARM64_REG_WIDTH) {
      neon_u32 data_vec = vld1q_u32(reinterpret_cast<const _u32*>(&this->data_[i]));
      ...
      vst1q_u32(&this->data_[i], asin_vec);
    }
  }
#pragma omp parallel
  for (; i < this->data_.size(); ++i) {
