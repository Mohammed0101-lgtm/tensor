...
tensor<_Tp> tensor<_Tp>::neon_sum(const index_type axis) const {
  if (axis < 0 or axis >= static_cast<index_type>(this->shape_.size())) {
  ...
  ret_sh[axis]      = 1;
  index_type ret_size =
      std::accumulate(ret_sh.begin(), ret_sh.end(), 1, std::multiplies<index_type>());
  data_t ret_data(ret_size, value_type(0.0f));

  const index_type axis_size  = this->shape_[axis];
  const index_type outer_size = this->compute_outer_size(axis);
  const index_type inner_size = this->size(0) / (outer_size * axis_size);
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    index_type i = 0;
    for (; i < static_cast<index_type>(this->data_.size()); ++i) {
      std::vector<index_type> orig(this->shape_.size());
      index_type              index = i;
      index_type              j     = static_cast<index_type>(this->shape_.size()) - 1;

      for (; j >= 0; j--) {
        orig[j] = index % this->shape_[j];
        index /= this->shape_[j];
      }
      ...
      for (j = static_cast<index_type>(this->shape_.size()) - 1; j >= 0; j--) {
        ret_index += orig[j] * st;
        st *= ret_sh[j];
      }
