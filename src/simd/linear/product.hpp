...
tensor<_Tp> tensor<_Tp>::neon_cross_product(const tensor& other) const {
  ...
  if (!equal_shape(this->shape(), shape_type({3})) or !equal_shape(other.shape(), shape_type({3})))
    throw shape_error("Cross product can only be performed on 3-element vectors");

  tensor ret({3});

  if constexpr (std::is_floating_point_v<value_type>) {
    neon_f32 a      = vld1q_f32(reinterpret_cast<const _f32*>(this->data_.data()));
    ...
    neon_s32 a      = vld1q_s32(reinterpret_cast<const _s32*>(this->data_.data()));
    ...
    neon_u32 a      = vld1q_u32(reinterpret_cast<const _u32*>(this->data_.data()));
