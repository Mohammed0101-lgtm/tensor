...
tensor<_Tp> tensor<_Tp>::neon_cross_product(const tensor& __other) const {
  ...
  if (!__equal_shape(this->shape(), shape_type({3})) ||
      !__equal_shape(__other.shape(), shape_type({3})))
    throw __shape_error__("Cross product can only be performed on 3-element vectors");
...
tensor<_Tp> tensor<_Tp>::neon_dot(const tensor& __other) const {
  ...
  if (__equal_shape(this->__shape_, shape_type({1})) &&
      __equal_shape(__other.shape(), shape_type({1})))
