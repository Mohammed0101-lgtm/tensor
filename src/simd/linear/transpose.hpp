...
tensor<_Tp> tensor<_Tp>::neon_transpose() const {
  if (!equal_shape(this->shape_, shape_type({this->shape_[0], this->shape_[1]})))
      ...
      for (index_type j = 0; j < cols; j += _ARM64_REG_WIDTH) {
        if (i + _ARM64_REG_WIDTH <= rows and j + _ARM64_REG_WIDTH <= cols) {
          ...
          for (index_type ii = i;
            ...
            for (index_type jj = j;
                 jj < std::min(static_cast<index_type>(j + _ARM64_REG_WIDTH), cols); ++jj) {
              ret.at({jj, ii}) = this->at({ii, jj});
            }
          }
        }
      }
    ...
    for (index_type i = 0; i < rows; i += _ARM64_REG_WIDTH) {
      for (index_type j = 0; j < cols; j += _ARM64_REG_WIDTH) {
        if (i + _ARM64_REG_WIDTH <= rows and j + _ARM64_REG_WIDTH <= cols) {
          ...
          for (index_type k = 0; k < _ARM64_REG_WIDTH; ++k)
            input.val[k] =
                vld1q_s32(reinterpret_cast<const _s32*>(&this->data_[(i + k) * cols + j]));
    ...
    for (index_type i = 0; i < rows; i += _ARM64_REG_WIDTH) {
      for (index_type j = 0; j < cols; j += _ARM64_REG_WIDTH) {
        if (i + _ARM64_REG_WIDTH <= rows and j + _ARM64_REG_WIDTH <= cols) {
          ...
          for (index_type k = 0; k < _ARM64_REG_WIDTH; ++k)
            input.val[k] =
                vld1q_u32(reinterpret_cast<const _u32*>(&this->data_[(i + k) * cols + j]));
