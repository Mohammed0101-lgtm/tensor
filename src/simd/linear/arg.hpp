...
tensor<typename tensor<_Tp>::index_type> tensor<_Tp>::neon_argmax_(index_type dim) const {
  ...
  ret.shape_ = ret_sh;
  ret.data_.resize(this->computeSize(ret_sh), 0);
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    for (i = 0; i < outer_size; ++i) {
      ...
      for (; j < inner_size; ++j) {
        ...
        for (; k + _ARM64_REG_WIDTH <= this->shape_[dim]; k += _ARM64_REG_WIDTH) {
          neon_f32 data_vec = vld1q_f32(reinterpret_cast<const _f32*>(
              &this->data_[(i * this->shape_[dim] + k) * inner_size + j]));
        ...
        for (; k < this->shape_[dim]; ++k) {
          _f32 v = this->data_[(i * this->shape_[dim] + k) * inner_size + j];
          if (v > max_value) {
            max_value = v;
            max_index = k;
          }
    ...
    for (i = 0; i < outer_size; ++i) {
      ...
      for (; j < inner_size; ++j) {
        ...
        for (; k + _ARM64_REG_WIDTH <= this->shape_[dim]; k += _ARM64_REG_WIDTH) {
          neon_s32 data_vec = vld1q_s32(reinterpret_cast<const _s32*>(
              &this->data_[(i * this->shape_[dim] + k) * inner_size + j]));
        ...
        for (; k < this->shape_[dim]; ++k) {
          _s32 v = this->data_[(i * this->shape_[dim] + k) * inner_size + j];
    ...
    for (i = 0; i < outer_size; ++i) {
      ...
      for (; j < inner_size; ++j) {
        ...
        for (; k + _ARM64_REG_WIDTH <= this->shape_[dim]; k += _ARM64_REG_WIDTH) {
          neon_u32 data_vec = vld1q_u32(reinterpret_cast<const _u32*>(
              &this->data_[(i * this->shape_[dim] + k) * inner_size + j]));
        ...
        for (; k < this->shape_[dim]; ++k) {
          _u32 v = this->data_[(i * this->shape_[dim] + k) * inner_size + j];
  ...
  for (i = 0; i < outer_size; ++i) {
    ...
    for (; j < inner_size; ++j) {
      ...
      value_type max_value = this->data_[i * this->shape_[dim] * inner_size + j];
      index_type k         = 1;
      for (; k < this->shape_[dim]; ++k) {
...
tensor<_Tp> tensor<_Tp>::neon_argmax(index_type dim) const {
  ...
  ret_sh.erase(ret_sh.begin() + dim);
  ret.shape_ = ret_sh;
  ret.data_.resize(this->computeSize(ret_sh), value_type(0));
...
template <class _Tp>
tensor<typename tensor<_Tp>::index_type> tensor<_Tp>::neon_argsort(index_type d,
